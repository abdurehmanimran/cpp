...
Account::Account() : id(p_id++), balance(0), name("Empty") {};
Account::Account(string accountName, uint64_t passCode)
...
uint64_t Account::getId() const { return id; }
...
// -> Transaction Class <-
// Constructor
Transaction::Transaction(Account &sender, Account &receiver,
...
ostream &operator<<(ostream &stream, const Transaction &transac) {
...
Bank::Bank() : totalAccounts() {};

void Bank::newAccount(string accountName, uint64_t initialDesposit) {
  uint64_t pin;
  cout << "Enter the pin: ";
  cin >> pin;
...
void Bank::transferMoney(uint64_t sender, uint64_t receiver, uint64_t amount) {
...
Account &Bank::getAccountById(uint64_t id) { return map.at(id); }

void Bank::displayOptions() {
...
bool Bank::isLoggedIn() { return loggedIn != 0; }

Account &Bank::getLoggedInAccount() { return getAccountById(loggedIn); };

void Bank::logOut() { loggedIn = 0; }
...
void Bank::deleteAccount(uint64_t id) { map.erase(id); }
